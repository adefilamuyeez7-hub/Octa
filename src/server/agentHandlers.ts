...
function buildOpenResponsesInput(input: AgentRequest): string | OpenResponsesMessageItem[] {
  const history = (input.history ?? []).filter(
    (entry): entry is { from: "you" | "agent"; text: string } =>
      (entry.from === "you" || entry.from === "agent") && typeof entry.text === "string" && entry.text.trim().length > 0,
  );
...
async function callMoltbotWebhook(input: AgentRequest, webhookUrl: string, apiKey?: string): Promise<AgentResult> {
